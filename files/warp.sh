...
before_showinfo() {
    ...
    [[ $(curl -s4m8 https://chat.openai.com/ -I | grep "text/plain") != "" ]] && chatgpt4="${RED}无法访问${PLAIN}" || chatgpt4="${GREEN}支持访问${PLAIN}"
    [[ $(curl -s6m8 https://chat.openai.com/ -I | grep "text/plain") != "" ]] && chatgpt6="${RED}无法访问${PLAIN}" || chatgpt6="${GREEN}支持访问${PLAIN}"
    if [[ -n $cli_port ]]; then
        [[ $(curl -sx socks5h://localhost:$cli_port https://chat.openai.com/ -I | grep "text/plain") != "" ]] && chatgpt_cli="${RED}无法访问${PLAIN}" || chatgpt_cli="${GREEN}支持访问${PLAIN}"
    fi
    if [[ -n $wireproxy_port ]]; then
        [[ $(curl -sx socks5h://localhost:$wireproxy_port https://chat.openai.com/ -I | grep "text/plain") != "" ]] && chatgpt_wireproxy="${RED}无法访问${PLAIN}" || chatgpt_wireproxy="${GREEN}支持访问${PLAIN}"
    fi
